...
This chart deploys real-time Node Resource Interface (NRI) plugin.
...
- Container runtime:
  - containerD:
    ...
    - Enable NRI feature by following
      ...
      ```sh
      helm install my-real-time nri-plugins/nri-resource-policy-real-time --set nri.runtime.patchConfig=true --namespace kube-system
      ```
  ...
  - CRI-O
    ...
    - Enable NRI feature by following
      ...
      ```sh
      helm install my-real-time nri-plugins/nri-resource-policy-real-time --namespace kube-system --set nri.runtime.patchConfig=true
      ```

## Installing the Chart

Path to the chart: `nri-resource-policy-real-time`.

```sh
helm repo add nri-plugins https://containers.github.io/nri-plugins
helm install my-real-time nri-plugins/nri-resource-policy-real-time --namespace kube-system
```

The command above deploys real-time NRI plugin on the Kubernetes cluster
...
```sh
# Install the real-time plugin with custom values provided using the --set option
helm install my-real-time nri-plugins/nri-resource-policy-real-time --namespace kube-system --set nri.runtime.patchConfig=true
```

```sh
# Install the real-time plugin with custom values specified in a custom values.yaml file
...
helm install my-real-time nri-plugins/nri-resource-policy-real-time --namespace kube-system -f myPath/values.yaml
```

## Uninstalling the Chart

To uninstall the real-time plugin run the following command:

```sh
helm delete my-real-time --namespace kube-system
```
...
| ------------------------ | ----------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------- |
| `image.name`             | [ghcr.io/containers/nri-plugins/nri-resource-policy-real-time](https://ghcr.io/containers/nri-plugins/nri-resource-policy-real-time)    | container image name                     |
...
| `config`                 | see [helm chart values](tree:/deployment/helm/real-time/values.yaml) for the default configuration                       | plugin configuration data                            |
